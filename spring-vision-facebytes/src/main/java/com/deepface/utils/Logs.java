...
public final class Logs {
    ...
    private static String format(String level, String component, String message, Throwable t, Map<String, ?> context) {
        ...
        sb.append('{');
        field(sb, "timestamp", DateTimeFormatter.ISO_INSTANT.format(Instant.now())); sb.append(',');
        field(sb, "level", level); sb.append(',');
        field(sb, "component", component); sb.append(',');
        field(sb, "message", message); sb.append(',');
        // Correlation fields from MDC if present
        field(sb, "correlation_id", nullToEmpty(MDC.get("correlation_id"))); sb.append(',');
        field(sb, "user_id", nullToEmpty(MDC.get("user_id"))); sb.append(',');
        ...
        if (t != null) {
            sb.append(',');
            sb.append("\"error\":{");
            field(sb, "type", t.getClass().getName()); sb.append(',');
