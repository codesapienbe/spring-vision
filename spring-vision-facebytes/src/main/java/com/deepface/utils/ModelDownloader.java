...
public final class ModelDownloader {
    ...
    private static final java.util.Map<String, String> MODEL_URLS = com.deepface.models.ModelUrls.defaults();
    ...
    private static void downloadModel(ModelType modelType) throws DeepFaceException {
        ...
        try {
            ...
            if (!verifyModelIntegrity(targetPath, modelType)) {
                ...
                try { Files.deleteIfExists(targetPath.resolveSibling(targetPath.getFileName().toString() + ".sha256")); } catch (Exception ignored) {}
            ...
            for (int attempt = 1; attempt <= MAX_ATTEMPTS_PER_URL; attempt++) {
                try {
                    ...
                    try {
                        ...
                        try { Files.writeString(sidecar, actual); } catch (Exception ignored) {}
                        ...
                        try { Files.move(tmpPath, targetPath, java.nio.file.StandardCopyOption.REPLACE_EXISTING, java.nio.file.StandardCopyOption.ATOMIC_MOVE); } catch (Exception moveEx) {
                        ...
                        try { Files.writeString(finalSidecar, actual); } catch (Exception ignored) {}

                        // success
                        return;
                    } finally {
                        // Ensure tmp cleanup in case of failures
                        try { Files.deleteIfExists(targetPath.resolveSibling(targetPath.getFileName().toString() + ".tmp")); } catch (Exception ignored) {}
                    }
                    ...
                    // exponential backoff
                    try { Thread.sleep(1000L * attempt); } catch (InterruptedException ie2) { Thread.currentThread().interrupt(); throw new IOException("Download interrupted", ie2); }
                } catch (Exception e) {
