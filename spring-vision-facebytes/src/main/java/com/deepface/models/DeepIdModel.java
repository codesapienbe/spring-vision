...
public final class DeepIdModel {
    ...
    public float[] generateEmbedding(BufferedImage face, int targetSize) {
        ...
        // No ONNX yet; deterministic fallback 160-dim (arbitrary but stable)
        return l2normalize(mockEmbedding(resized, 160));
    }
    ...
    private static float[] mockEmbedding(BufferedImage img, int size) {
        long seed = 1L;
        try {
            MessageDigest md = MessageDigest.getInstance("SHA-256"); int w = img.getWidth(), h = img.getHeight(); int stepY = Math.max(1, h / 16), stepX = Math.max(1, w / 16);
