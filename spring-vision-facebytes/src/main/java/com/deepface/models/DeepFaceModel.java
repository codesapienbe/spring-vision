...
public final class DeepFaceModel {
    ...
    public float[] generateEmbedding(BufferedImage face, int targetSize) {
        ...
        try {
            ...
            Logs.warn("DeepFaceModel", "onnx.unavailable", java.util.Map.of());
            try {
                ...
                int hash = resized.hashCode(); final int dim = 256; float[] emb = new float[dim]; for (int i = 0; i < dim; i++) emb[i] = (float) Math.sin(hash + i * 0.1); return l2normalize(emb);
            }
