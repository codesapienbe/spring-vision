...
@DisplayName("Integration: ActionRecognitionCapability")
public class ActionRecognitionCapabilityIntegrationTest {
    ...
    @Test
    void recognizeActionsReturnsList() throws IOException {
        ImageData img = TestImageUtils.createPersonSilhouetteImage(320, 240);
        List<Detection> res;
        if (backend instanceof ActionRecognitionCapability cap) {
            res = cap.recognizeActions(img);
