...
/**
 * Custom translator for RetinaFace face detection model.
 *
 * <p>RetinaFace is a high-accuracy face detection model that provides
 * precise face localization with landmark detection capabilities.</p>
 *
 * <p>Based on the working DJL RetinaFace example with optimized parameters.</p>
 *
...
public class FaceDetectionTranslator implements Translator<Image, DetectedObjects> {
    ...
    /**
     * Creates a new FaceDetectionTranslator with RetinaFace parameters.
     *
     * @param confThresh confidence threshold for detections
     * @param nmsThresh NMS threshold for filtering overlapping boxes
     * @param variance variance for anchor decoding
     * @param topK maximum number of detections to keep before NMS
     * @param scales anchor scales for different feature levels
     * @param steps feature map strides
     */
    public FaceDetectionTranslator(double confThresh, double nmsThresh, double[] variance,
                                   int topK, int[][] scales, int[] steps) {
